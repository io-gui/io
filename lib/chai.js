...
let config = require('./chai/config');
...
let assertion = require('./chai/assertion');
...
let core = require('./chai/core/assertions');
exports.use(core);
...
let expect = require('./chai/interface/expect');
exports.use(expect);
...
let should = require('./chai/interface/should');
exports.use(should);
...
let assert = require('./chai/interface/assert');
exports.use(assert);
...
module.exports = function (chai, _) {
  ...
  function include (val, msg) {
    ...
    switch (objType) {
      ...
      case 'map':
        let isEql = isDeep ? _.eql : SameValueZero;
  ...
  Assertion.addProperty('empty', function () {
    ...
    switch (_.type(val).toLowerCase()) {
      ...
      case 'function':
        let msg = flagMsg + '.empty was passed a function ' + _.getName(val);
  ...
  function assertInstanceOf (constructor, msg) {
    ...
    try {
      let isInstanceOf = target instanceof constructor;
    } catch (err) {
  ...
  function assertThrows (errorLike, errMsgMatcher, msg) {
    ...
    if (errorLike && caughtErr) {
      ...
      if (errorLike instanceof Error) {
        let isCompatibleInstance = _.checkError.compatibleInstance(caughtErr, errorLike);
      ...
      let isCompatibleConstructor = _.checkError.compatibleConstructor(caughtErr, errorLike);
    ...
    if (caughtErr && errMsgMatcher !== undefined && errMsgMatcher !== null) {
      ...
      let isCompatibleMessage = _.checkError.compatibleMessage(caughtErr, errMsgMatcher);
  ...
  Assertion.addMethod('members', function (subset, msg) {
    ...
    let cmp = flag(this, 'deep') ? _.eql : undefined;
...
module.exports = function (chai, util) {
  ...
  let assert = chai.assert = function (express, errmsg) {
 ...
 /**
  ...
  assert.throws = function (fn, errorLike, errMsgMatcher, msg) {
    ...
    let assertErr = new Assertion(fn, msg, assert.throws, true)
   ...
   *
   *     let obj = { val: 10 };
   *     let fn = function() { console.log('foo'); };
...
let addLengthGuard = require('./addLengthGuard');
let chai = require('../../chai');
let flag = require('./flag');
let proxify = require('./proxify');
let transferFlags = require('./transferFlags');
  ...
  Object.defineProperty(ctx, name,
    { get: function chainableMethodGetter() {
        ...
        let chainableMethodWrapper = function () {
          ...
          let newAssertion = new chai.Assertion();
...
let chai = require('../../chai');
let flag = require('./flag');
let isProxyEnabled = require('./isProxyEnabled');
let transferFlags = require('./transferFlags');
...
let getOwnEnumerablePropertySymbols = require('./getOwnEnumerablePropertySymbols');
...
let getEnumerableProperties = require('./getEnumerableProperties');
let config = require('../config');
...
let builtins = ['__flags', '__methods', '_obj', 'assert'];
...
function exclude () {
  let excludes = [].slice.call(arguments);
...
let functionNameMatch = /\s*function(?:\s|\s*\/\*[^(?:*\/)]+\*\/\s*)*([^\(\/]+)/;
function getFunctionName(constructorFn) {
  let name = '';
  if (typeof constructorFn.name === 'undefined') {
    // Here we run a polyfill if constructorFn.name is not defined
    let match = String(constructorFn).match(functionNameMatch);
    if (match) {
      name = match[1];
    }
  } else {
...
let MemoizeMap = typeof WeakMap === 'function' ? WeakMap : FakeMap;
/*!
...
function extensiveDeepEqual(leftHandOperand, rightHandOperand, options) {
  ...
  let leftHandType = type(leftHandOperand);
...
function getFuncName(aFunc) {
  ...
  if (typeof Function.prototype.name === 'undefined' && typeof aFunc.name === 'undefined') {
    ...
    let match = toString.call(aFunc).match(functionNameMatch);
    if (match) {
      name = match[1];
    }
  } else {
 ...
 *
 *      let parsed = parsePath('myobject.property.subprop');
 *
 * ### Paths:
 *
 ...
 *
 *      let value = getPathValue(obj, parsed);
 *
 ...
 *
 * ```js
 * let properties = require('tea-properties');
 ...
 * MIT Licensed
 */
let promiseExists = typeof Promise === 'function';

/* eslint-disable no-undef */
let globalObject = typeof self === 'object' ? self : global; // eslint-disable-line id-blacklist

let symbolExists = typeof Symbol !== 'undefined';
let mapExists = typeof Map !== 'undefined';
let setExists = typeof Set !== 'undefined';
let weakMapExists = typeof WeakMap !== 'undefined';
let weakSetExists = typeof WeakSet !== 'undefined';
let dataViewExists = typeof DataView !== 'undefined';
let symbolIteratorExists = symbolExists && typeof Symbol.iterator !== 'undefined';
let symbolToStringTagExists = symbolExists && typeof Symbol.toStringTag !== 'undefined';
let setEntriesExists = setExists && typeof Set.prototype.entries === 'function';
let mapEntriesExists = mapExists && typeof Map.prototype.entries === 'function';
let setIteratorPrototype = setEntriesExists && Object.getPrototypeOf(new Set().entries());
let mapIteratorPrototype = mapEntriesExists && Object.getPrototypeOf(new Map().entries());
let arrayIteratorExists = symbolIteratorExists && typeof Array.prototype[Symbol.iterator] === 'function';
let arrayIteratorPrototype = arrayIteratorExists && Object.getPrototypeOf([][Symbol.iterator]());
let stringIteratorExists = symbolIteratorExists && typeof String.prototype[Symbol.iterator] === 'function';
let stringIteratorPrototype = stringIteratorExists && Object.getPrototypeOf(''[Symbol.iterator]());
let toStringLeftSliceLength = 8;
let toStringRightSliceLength = -1;
/**
 * ### typeOf (obj)
 *
