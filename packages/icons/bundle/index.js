import { Node as r, Register as s, ReactiveProperty as e, IoElement as a } from "@io-gui/core";
var o = Object.getOwnPropertyDescriptor, p = (l, c, i, M) => {
  for (var h = M > 1 ? void 0 : M ? o(c, i) : c, g = l.length - 1, d; g >= 0; g--)
    (d = l[g]) && (h = d(h) || h);
  return h;
};
const z = {};
let V = class extends r {
  registerIcons(l, c) {
    ...
    i.innerHTML = c, i.querySelectorAll("[id]").forEach((M) => {
      z[l] = z[l] || {}, z[l][M.id] = M.outerHTML;
    });
  }
  getIcon(l) {
    const c = z[l.split(":")[0]];
    if (c) {
      const i = l.split(":")[1];
      if (c[i])
        return `<svg viewBox="0 0 24 24" preserveAspectRatio="xMidYMid meet">${c[i].replace(' id="', ' class="icon-id-')}</svg>`;
    }
...
v.registerIcons("io", n);
var L = Object.defineProperty, C = Object.getOwnPropertyDescriptor, H = (l, c, i, M) => {
  for (var h = M > 1 ? void 0 : M ? C(c, i) : c, g = l.length - 1, d; g >= 0; g--)
    (d = l[g]) && (h = (M ? d(c, i, h) : d(h)) || h);
  return M && h && L(c, i, h), h;
};
let t = class extends a {
  ...
  valueChanged() {
    this.value.search(":") !== -1 ? this.innerHTML = v.getIcon(this.value) : this.innerText = this.value;
  }
};
H([
  e({ value: "", type: String, reflect: !0 })
], t.prototype, "value", 2);
H([
  e({ value: !1, type: Boolean, reflect: !0 })
], t.prototype, "stroke", 2);
H([
  e({ value: "small", type: String, reflect: !0 })
], t.prototype, "size", 2);
t = H([
  s
], t);
const S = function(l) {
  return t.vConstructor(l);
};
export {
  z as IconsetDB,
  v as IconsetSingleton,
  t as IoIcon,
  S as ioIcon
};
