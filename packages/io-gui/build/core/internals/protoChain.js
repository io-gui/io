...
import { hardenListenerDefinition } from './eventDispatcher';
export const propertyDecorators = new WeakMap();
const NON_OBSERVED = [String, Number, Boolean, Date, RegExp, Map, Set, WeakMap, WeakSet];
function isNonIoNodeConstructor(constructor) {
    if (typeof constructor !== 'function')
        return false;
    let proto = constructor.prototype;
    while (proto) {
        if (NON_OBSERVED.includes(constructor))
            return false;
        if (proto.constructor.name === 'IoNodeMixinConstructor')
            return false;
...
function isNonIoNodeObject(value) {
    return (typeof value === 'object' && value !== null && !value._isIoNode);
}
function isIoNodeObjectConstructor(constructor) {
    if (typeof constructor !== 'function')
        return false;
    let proto = constructor.prototype;
    while (proto) {
...
export class ProtoChain {
     ...
     */
    addStaticProperties(properties = {}, prevHash = '') {
        ...
        if (newHash !== prevHash) {
            for (const name in properties) {
                if (!this.properties[name])
                    this.properties[name] = protoProperties[name];
                else
                    this.properties[name].assign(protoProperties[name]);
                prevHash = newHash;
            }
        }
        return prevHash;
    ...
    getObservedObjectProperties() {
        const observedObjectProperties = [];
    ...
    getObservedIoNodeProperties() {
        const observedIoNodeProperties = [];
        for (const name in this.properties) {
            const value = this.properties[name].value;
            const type = this.properties[name].type;
