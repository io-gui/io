...
export class Binding<T extends unknown> {
  ...
  constructor(node: Node | IoElement, property: string) {
    debug: {
      if (!node._isNode && !node._isIoElement) console.warn('Source node is not a Node or IoElement instance!');
  ...
  addTarget(target: Node | IoElement, property: string) {
    ...
    debug: {
      if (!target._isNode && !target._isIoElement) console.warn('Target node is not a Node or IoElement instance!');
