...
let TestContent = class TestContent extends IoElement {
    changed() {
...
], TestContent.prototype, "content", void 0);
TestContent = __decorate([
    Register
], TestContent);
describe('IoSelector', () => {
    let container;
    ...
    describe('Re-rendering on deep element changes', () => {
        it('should re-render selected element when elements array is updated with same id but different props', async () => {
            ...
            container.appendChild(selector);
            await nextQueue();
            const initialChild = selector.querySelector('test-content');
            ...
            await nextQueue();
            const updatedChild = selector.querySelector('test-content');
    ...
    describe('Caching modes', () => {
        it('should not cache elements when caching is "none"', async () => {
            ...
            container.appendChild(selector);
            await nextQueue();
            const firstChild = selector.querySelector('test-content');
            selector.selected = 'second';
            await nextQueue();
            selector.selected = 'first';
            await nextQueue();
            const newFirstChild = selector.querySelector('test-content');
        ...
        it('should cache elements when caching is "reactive"', async () => {
            ...
            selector.selected = 'first';
            await nextQueue();
            const cachedFirstChild = selector.querySelector('test-content');
        ...
        it('should reuse cached element (cache preserves original props)', async () => {
            ...
            container.appendChild(selector);
            await nextQueue();
            const cachedChild = selector.querySelector('test-content');
            expect(cachedChild.content).toBe('cached initial');
            selector.elements = [
                { tag: 'test-content', props: { id: 'test-id', content: 'cached updated' } }
            ];
            await nextQueue();
            const reusedChild = selector.querySelector('test-content');
