...
export class IoBenchmarksDemo extends IoElement {
  ...
  changed() {
    ...
    this.render([
      ...
      ...this.#report.files.flatMap(file =>
        file.groups.map(group => this.renderGroup(group))
      ),
    ])
  }

  renderGroup(group: { fullName: string; benchmarks: Array<Record<string, number | string>> }) {
