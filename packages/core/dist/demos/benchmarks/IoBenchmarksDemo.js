...
function averageMargin(baseline, current) {
...
function formatDeltaPercent(baseline, current) {
    if (baseline === null || current === null)
        return 'NA';
...
function metricValue(benchmark, key) {
    if (!benchmark)
        return undefined;
    ...
    ready() {
        Promise.all([
            fetch(RESULTS_URL).then(res => {
                if (!res.ok)
                    throw new Error(`${RESULTS_URL}: ${res.status} ${res.statusText}`);
                return res.json();
            }),
    ...
    mutated() {
        ...
        this.render([
            ...
            this.#baselineMissing
                ...
                : null,
            ...collectGroupNames(this.#report, this.#baseline).map(groupName => this.renderGroup(groupName, baselineIndex)),
        ]);
    }
    renderGroup(groupName, baselineIndex) {
                ...
                table([
                    ...
                    tbody(benchmarkNames.map(name => {
                        ...
                        return tr([
                            ...
                            td({ class: 'numeric group-start' }, formatMargin(averageMargin(metricValue(baseline, MARGIN_KEY), metricValue(current, MARGIN_KEY)))),
                        ]);
                    })),
                ]),
            ]),
...
export const ioBenchmarksDemo = (arg0) => IoBenchmarksDemo.vConstructor(arg0);
