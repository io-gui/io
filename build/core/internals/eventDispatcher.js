 ...
 */
export const hardenListenerDeclaration = (def) => {
 ...
 */
export const assignListenerDeclaration = (defs, srcDef) => {
    const i = defs.findIndex(def => def[0] === srcDef[0]);
    if (i !== -1) {
        if (defs[i][1])
            defs[i][1] = Object.assign(defs[i][1], srcDef[1]);
        else if (srcDef[1])
            defs[i][1] = srcDef[1];
    }
    else {
...
export const listenerFromDefinition = (node, def) => {
    debug: {
        if (typeof def[0] !== 'string' && typeof def[0] !== 'function')
            console.warn('listenerFromDefinition: Invalid listener type');
        if (def[1]) {
...
export class EventDispatcher {
    ...
    addEventListener(name, listener, options) {
        ...
        debug: {
            if (l !== -1)
                console.warn(`EventDispatcher.addEventListener: Listener ${name} already added!`);
            if (typeof listener !== 'function')
                console.warn('EventDispatcher.addEventListener: Invalid listener type!');
            if (options) {
                if (typeof options !== 'object')
                    console.warn('EventDispatcher.addEventListener: Invalid listener options type');
            ...
            if (this.propListeners[name]) {
                debug: {
                    if (this.propListeners[name].length > 1)
                        console.warn(`EventDispatcher.dispathEvent: PropListeners[${name}] array too long!`);
                }
