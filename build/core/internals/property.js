 ...
 */
export const assignProtoProperty = (def, srcDef) => {
    if (srcDef.value !== undefined)
        def.value = srcDef.value;
    if (srcDef.type !== undefined)
        def.type = srcDef.type;
    if (srcDef.reflect !== 0)
        def.reflect = srcDef.reflect;
    if (srcDef.notify !== true)
        def.notify = srcDef.notify;
    if (srcDef.observe !== false)
        def.observe = srcDef.observe;
    if (srcDef.binding !== undefined)
        def.binding = srcDef.binding;
};
/**
