    ...
    run() {
        describe('MenuItem', () => {
            ...
            it('Should initialize suboptions from constructor arguments', () => {
                ...
                chai.expect(item.options[0].value).to.be.equal(1);
                chai.expect(item.options[0].label).to.be.equal('1');
                chai.expect(item.options[1].value).to.be.equal('2');
                chai.expect(item.options[1].label).to.be.equal('2');
                chai.expect(item.options[2].value).to.be.equal(null);
                chai.expect(item.options[2].label).to.be.equal('null');
                item = new MenuItem({ options: [1, { value: '2', label: 'two' }, undefined] });
                chai.expect(item.options[0].value).to.be.equal(1);
                chai.expect(item.options[0].label).to.be.equal('1');
                chai.expect(item.options[1].value).to.be.equal('2');
                chai.expect(item.options[1].label).to.be.equal('two');
                chai.expect(item.options[2].value).to.be.equal(undefined);
                chai.expect(item.options[2].label).to.be.equal('undefined');
