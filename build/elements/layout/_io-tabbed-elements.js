 ...
 **/
let IoSelectorTabs = class IoSelectorTabs extends IoElement {
    static get Style() {
        return /* css */ `
    :host {
      display: flex;
    ...
    elementsChanged() {
        if (this.filter === null) {
            this.setProperty('filter', this.elements.map((element) => { return element[1].name; }));
        }
    }
    editableChanged() {
...
let IoTabs = class IoTabs extends IoElement {
    ...
    changed() {
        ...
        for (let i = 0; i < this.filter.length; i++) {
            ...
            let _class = 'io-tab';
            if (selected)
                _class += ' io-selected-tab';
            if (this.dropIndex !== -1) { // && this.dropIndex !== currentIndex && this.dropIndex !== currentIndex + 1) {
                if (this.dropIndex === i)
                    _class += ' io-tab-insert-before';
                if (this.dropIndex === i + 1)
                    _class += ' io-tab-insert-after';
            }
            ...
            if (this.editable)
                button[1]['@pointerdown'] = this._onPointerdown;
            if (selected)
...
export { IoTabs };
let IoTabDragicon = class IoTabDragicon extends IoElement {
    static get Style() {
        return /* css */ `
    :host {
...
export { IoTabDragicon };
let IoTabDropzone = class IoTabDropzone extends IoElement {
    static get Style() {
        return /* css */ `
    :host {
      width: 100px;
...
IoTabDropzone = __decorate([
    Register
], IoTabDropzone);
export { IoTabDropzone };
//# sourceMappingURL=_io-tabbed-elements.js.map