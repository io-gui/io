...
//       if (config.type === Boolean) config.value = false;
//       if (config.type === Number) config.value = 0;
//       if (config.type === String) config.value = '';
//     }
//     this.value = config.value;
//     this.type = config.type;
//     this.observer = config.observer;
//     this.reflectToAttribute = config.reflectToAttribute;
...
class Io extends HTMLElement {
  ...
  constructor(props = {}) {
    ...
    const attributes = new Attributes(protochain);
    for (let att in attributes) {
      this.setAttribute(att, attributes[att]);
    }

    const handlers = this.__proto__.constructor.definedHandlers;
...
class UiMenuGroup extends Io {
  static get style() {
    return html`<style>:host {display: flex;visibility: hidden;flex-direction: column;position: absolute;transform: translateZ(0);top: 0;left: 0;background: white;white-space: nowrap;padding: 0.125em 0 0.25em 0;border: 1px solid #666;box-shadow: 1px 1px 2px rgba(0,0,0,0.33);min-width: 6em;}:host[expanded] {visibility: visible;}</style>`;
  }
  static get properties() {
    return {
      options: {
...
class UiLayout extends Io {
  static get style() {
    return html`<style>:host{display: flex;}</style>`;
  }
  static get properties() {
    return {
      elements: {
