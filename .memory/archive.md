...
- **category:technical** Production dynamic import of three demos failed: `Failed to resolve module specifier "three"`. Root cause: import map had `three/webgpu`, `three/tsl`, `three/addons/` but not bare `three`. `@io-gui/three` pulls OrbitControls which imports `from 'three'`. Fix: add `"three": "./packages/three/node_modules/three/build/three.webgpu.js"` to index.html and dev.html import maps (matches three.js webgpu examples convention).
- **category:technical** TodoMVC second-load crash from localStorage: chain of bugs — (1) ReactiveNode.toJSON skipped strings/booleans so items saved as `[{}]`; (2) NodeArray.applyJSON called `.applyJSON` on undefined index when array empty → throw; (3) Storage catch replaced model with raw JSON string → `model.items` undefined. Fixes: toJSON includes string/boolean; NodeArray.applyJSON grow/shrink/no-throw; TodoListModel.applyJSON hydrates wire items into NodeArray via setProperty.
- **category:technical** TodoMVC clearCompleted duplicate listener warning: `this.items = this.items.filter(...)` triggers `applyNodeArrayAssignment` which used `length=0`+`push` inside `withInternalOperation`. Proxy `length` setter skips listener removal during internal ops, but `push` re-adds listeners → duplicate `io-object-mutation` on kept items. Fix: use `nodeArray.splice(0, length, ...items)` which explicitly removes/adds listeners.
- **category:technical** Benchmark workflow split: `bench:baseline` writes `benchmarks/results-baseline.json`; `bench` compares against baseline and writes `results.json`. IoBenchmarksDemo loads both, renders per-metric baseline/current/Δ columns; missing baseline benchmarks or metric fields show NA.
- **category:technical** Bench scripts opened browser + watch mode: bench project had `browser: browserUnitConfig` and scripts lacked `--run`. Fix: Node-only bench project (exclude `*.browser.bench.ts`, `pool: forks`), separate `bench-browser` project, add `--run` to bench scripts.
- **category:technical** Added VDOM `text()` helper (`TEXT_TAG = '#text'`). `filterVDOMElements` normalizes plain strings to `text()`. Reconciliation/traverse switched from `host.children` to `host.childNodes` for mixed text+element siblings. `constructElement` returns `ChildNode` and creates `document.createTextNode` for `#text`. Tests + benchmarks added.
