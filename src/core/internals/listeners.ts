...
export type ListenerDefinitionWeak = keyof IoNode | EventListener | ListenerDefinition;
export type ListenerDefinition = [keyof IoNode | EventListener, AddEventListenerOptions?];

export const sanitizeListenerDefinition = (listenerDefinition: ListenerDefinition | ListenerDefinitionWeak): ListenerDefinition => {
  debug: {
    if (listenerDefinition instanceof Array) {
      if (typeof listenerDefinition[0] !== 'string' && typeof listenerDefinition[0] !== 'function') console.warn('Listeners: invalid listener type');
      ...
      if (typeof listenerDefinition !== 'string' && typeof listenerDefinition !== 'function') console.warn('Listeners: invalid listener type');
    }
  }
  return listenerDefinition instanceof Array ? listenerDefinition : [listenerDefinition];
};

export const assignListenerDefinition = (definitions: ListenerDefinition[], listenerDefinition: ListenerDefinition | ListenerDefinitionWeak) => {
  ...
  if (i !== -1) {
    if (definitions[i][1]) definitions[i][1] = Object.assign(definitions[i][1], newListenerDefinition[1]);
    else if (newListenerDefinition[1]) definitions[i][1] = newListenerDefinition[1];
  } else {
    definitions.push(newListenerDefinition);
  }
};

export const sanitizeListener = (node: IoNode, listenerDefinition: ListenerDefinition): Listener => {
  ...
  */
  removeEventListener(type: string, listener?: EventListener, options?: AddEventListenerOptions) {
    debug: {
      ...
      if (listener && typeof listener !== 'function') console.warn('EventDispatcher: invalid listener type!');
      if (options && typeof options !== 'object') console.warn('EventDispatcher: invalid options type!');
    }
    if (!listener) {
  ...
  dispatchEvent(type: string, detail: Record<string, any> = {}, bubbles = true, node: EventTarget | IoNode = this.__node) {
    ...
    if ((node instanceof EventTarget)) {
      ...
      if (this.__protoListeners[type] !== undefined) {
        for (let i = 0; i < this.__protoListeners[type].length; i ++) {
          const listener = this.__protoListeners[type][i];
          (listener[0] as EventListener).call(node, {detail: detail, target: node, path: [node]} as any);
        }
      }
      if (this.__propListeners[type] !== undefined) {
        debug: {
          if (this.__propListeners[type].length > 1) console.warn(`EventDispatcher: __propListeners[${type}] array too long!`);
        }
        const listener = this.__propListeners[type][0];
        (listener[0] as EventListener).call(node, {detail: detail, target: node, path: [node]} as any);
      }
