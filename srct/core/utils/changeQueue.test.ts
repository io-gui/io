  ...
  run() {
    describe('Queue', () => {
      it('Should have correct defaults', () => {
        this.init();
        chai.expect(typeof (this.changeQueue as any).__node).to.be.equal('object');
        chai.expect(typeof (this.changeQueue as any).__changes).to.be.equal('object');
        chai.expect((this.changeQueue as any).__changes.length).to.be.equal(0);
        chai.expect((this.changeQueue as any).__changes instanceof Array).to.be.equal(true);
        chai.expect((this.changeQueue as any).__dispatching).to.be.equal(false);
      });
      it('Should dispose correctly', () => {
        this.init();
        this.changeQueue.dispose();
        chai.expect(typeof (this.changeQueue as any).__node).to.be.equal('undefined');
        chai.expect(typeof (this.changeQueue as any).__changes).to.be.equal('undefined');
        chai.expect(typeof (this.changeQueue as any).__dispatching).to.be.equal('undefined');
      });
      it('Should trigger change events', () => {
        this.init();
        this.changeQueue.queue('test', 1, 0);
        this.changeQueue.queue('test', 2, 1);
        chai.expect((this.changeQueue as any).__changes.length).to.be.equal(1);
        ...
        chai.expect((this.changeQueue as any).__changes.length).to.be.equal(2);
        this.changeQueue.dispatch();
        chai.expect((this.changeQueue as any).__changes.length).to.be.equal(0);
